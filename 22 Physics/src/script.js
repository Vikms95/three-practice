...
const playSound = ( collision ) => {
  ...
  // Check how hard did the box hit so we can either play the sound or not
  const impactStrenght = collision.contact.getImpactVelocityAlongNormal()

  if ( impactStrenght > 1.5 ) {
    hitSound.volume = Math.random() / impactStrenght
...
debugObject.reset = () => {
  // Delete all objects from the scene and world
...
//createBox( 0.5, { x: 3, y: 3, z: 2 } )
//createBox( 0.5, { x: 2, y: 3, z: 7 } )
